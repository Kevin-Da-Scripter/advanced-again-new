...
@bot.tree.command(name="img", description="Generate images using AI.")
async def generate_image(interaction: discord.Interaction, prompt: str, model: str = None):
    # Asumsi: model_check_prompt_flash dan fungsi terkait sudah terdefinisi secara global
    await interaction.response.defer()
    
    # [PERUBAHAN 2: Pastikan /img menggunakan HUGGING_FACE_API yang dimuat dari env]
    # HUGGING_FACE_API sudah dimuat di awal file
    if not HUGGING_FACE_API: # Check if the API key is empty
        await interaction.followup.send("❌ API Key Hugging Face belum diatur di lingkungan (environment variables).")
        return

    # Logika pemeriksaan NSFW (jika terpotong, diasumsikan sudah ada di file asli)
    try:
        # model_check_prompt_flash (diambil dari model_V2 yang sudah diubah ke 2.5-flash)
        model_check_prompt_flash = genai.GenerativeModel(model_name="gemini-2.5-flash", generation_config=gen_config, system_instruction="Your only propose is to check if the user's prompt is a NSFW prompt, respond ONLY with 'NSFW' if the prompt is NSFW or 'OK' if not, DONT say anything else! and nothing more and nothing less!")
        response_check = model_check_prompt_flash.generate_content(prompt)
        
        if safegen:
            if "NSFW" in response_check.text.upper():
                await interaction.followup.send("🚫 Prompt Anda melanggar kebijakan SafeGen. Permintaan ditolak.")
                return

    except Exception as e:
        # print(f"Checking prompt Error: {e}")
        pass # Lanjutkan jika pemeriksaan gagal
    ...
    # Model name mapping (tetap sama dengan file asli)
    if model == "stabilityai/stable-diffusion-xl-base-1.0": model_name = "Stable Diffusion XL Base 1.0"
    elif model == f"{Image_Model}": model_name = f"{Image_Model_Name}"
    elif model == "ehristoforu/dalle-3-xl-v2": model_name = "DALL-E 3 XL V2"
    elif model == "black-forest-labs/FLUX.1-schnell": model_name = "FLUX.1 Schnell"
    elif model == "dataautogpt3/FLUX-anime2": model_name = "FLUX Anime 2"
    elif model == "Yntec/Chip_n_DallE": model_name = "Chip & DallE"
    elif model == "black-forest-labs/FLUX.1-dev": model_name = "Flux.1 DEV"
    elif model == "stabilityai/stable-diffusion-3-medium-diffusers": model_name = "Stable Diffusion 3 Medium Diffusers"
    elif model == "Shakker-Labs/AWPortrait-FL": model_name = "AWPortrait FL"
    elif model == "Shakker-Labs/FLUX.1-dev-LoRA-Garbage-Bag-Art": model_name = "FLUX.1 DEV LoRA Art"
    elif model == "Shakker-Labs/FLUX.1-dev-LoRA-playful-metropolis": model_name = "Flux.1 DEV LoRA Playful Metropolis Art"
    elif model == "Shakker-Labs/FLUX.1-dev-LoRA-Logo-Design": model_name = "Flux.1 DEV LoRA Logo Design"
    elif model == "Shakker-Labs/FLUX.1-dev-LoRA-add-details": model_name = "Flux.1 DEV LoRA Add Details"
    else: model_name = "Unkown Model"
    ...
    def handle_error(response):
        ...
        if response.status_code == 503:
            asyncio.run_coroutine_threadsafe(interaction.followup.send("Oopsies! Looks like our generator engine are taking a little snooze! Please try again later, and maybe bring some coffee"), bot.loop)
        elif response.status_code == 429:
            asyncio.run_coroutine_threadsafe(interaction.followup.send("Uh-oh! We've encountered a Rate Limit Error! Please try again later."), bot.loop)
        elif response.status_code == 500:
            asyncio.run_coroutine_threadsafe(interaction.followup.send("Uh-oh! We've encountered an Internal Server Error! Our processing engine is having a little meltdown. Please try again shortly."), bot.loop)
        else:
            asyncio.run_coroutine_threadsafe(interaction.followup.send(f"❌ Failed to generate image: {error_message}"), bot.loop)
        add_to_history_bot("Error", f"Image Generation failed: {response.status_code} - {error_message}")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                save_image(response)
                # Success
                add_to_history_bot(name, f"Image generated: {prompt} with model {model_name}")
                file_to_send = discord.File("system/RAM/gen-image/generated_image.png", filename="image.png")
                embed = discord.Embed(title=f"✨ Generated Image: {prompt[:250]}...", color=embed_colors)
                embed.set_footer(text=f"Model: {model_name} | Generated by: {member_name}")
                embed.set_image(url="attachment://image.png") 

                await interaction.followup.send(embed=embed, file=file_to_send)
                return
            elif response.status_code == 503 and attempt < max_retries - 1:
                # Retry for 503 (Service Unavailable/Loading)
                wait_time = backoff_factor ** attempt
                # print(f"Model loading or unavailable. Retrying in {wait_time}s...")
            ...
            if attempt < max_retries - 1:
                ...
                await interaction.followup.send("❌ Request timed out after several retries.")
                add_to_history_bot("Error", "Image Generation failed: Request timed out.")
                return
        except Exception as e:
            # print(f"An unexpected error occurred during image generation: {e}")
            await interaction.followup.send("❌ Terjadi kesalahan tak terduga saat memproses gambar.")
            add_to_history_bot("Error", f"Image Generation failed: {str(e)}")
            return

    await interaction.followup.send("❌ Gagal menghasilkan gambar setelah mencoba beberapa kali. Server sibuk.")
    add_to_history_bot("Error", "Image Generation failed: Max retries reached.")
